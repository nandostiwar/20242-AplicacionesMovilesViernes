...
const compareLogin = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error de credenciales' });
    }
};
...
const updatePassword = async (req, res) => {
    try {
        ...
        if (userIndex === -1) {
            return res.status(404).json({ error: 'Usuario existe' });
        }
        ...
        res.status(500).json({ error: 'Error en la solicitud de actualización de contraseña' });
    }
};
